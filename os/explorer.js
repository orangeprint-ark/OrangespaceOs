...
export async function init(OS) {
  OS.registerApp('explorer', async () => {
    ...
    uploadBtn.style.background = '#0a84ff';
    uploadBtn.style.border = 'none';
    uploadBtn.style.borderRadius = '8px';
    uploadBtn.style.color = 'white';
    uploadBtn.style.fontWeight = '600';
    uploadBtn.style.cursor = 'pointer';
    uploadBtn.onmouseenter = () => (uploadBtn.style.background = '#006edc');
    uploadBtn.onmouseleave = () => (uploadBtn.style.background = '#0a84ff');

    topButtons.append(newFolderBtn, refreshBtn, uploadBtn);
    ...
    fileInput.onchange = async (event) => {
      ...
      for (const file of files) {
        ...
        await userStorage.saveEntry(path, text, false);
      }
